...
function ds = derivative(X,R_1,R_20,b,R_pos,R_neg,Rs,N,t)
...
ds = zeros(2*N,1);   %initialize vector containing derivatives
    ...
    for i = 1:2*N  %for each entry in derivative vector,
        ...
        - (  (R_1 / R_neg) - (Rs(index) / R_pos) ) * ( X(i+2) + X(i-2) );
        end
    end
end
